...
* **LocalStorage Persistence:** Automatically saves progress in the browser under the key `hyre_builder_autosave` to prevent data loss on refresh.
* **Dynamic Favicon:** The site icon automatically updates its color in real-time to match your chosen brand accent.
* **Single-File Export:** Packages the entire portfolio—including all internal CSS and logic—into a single, standalone `hyre_portfolio.html` file.

## 🛠️ Built With

* **Vanilla JavaScript:** Lightweight state management and dynamic DOM rendering with zero dependencies.
* **CSS3 Custom Properties:** Fully themed using CSS variables (`--accent`, `--bg`) for instant style updates.
...
2.  **Style:** Choose your accent and background colors; the UI and favicon update instantly to reflect your brand.
3.  **Auto-Save:** Every keystroke triggers a save to your browser's local storage, allowing you to pick up where you left off.
4.  **Export:** Click "Export Site" to download a production-ready HTML file with all styles and scripts embedded.
